<?php
	...
	if($mod == "barang" AND $act == "simpan")
	{
		...
		if(empty($kode_barang)) {
			$q = mysqli_query($id_mysql,"SELECT MAX(RIGHT(kode_barang,5)) AS kodebrg
		...
		mysqli_query($id_mysql,"INSERT INTO tb_barang(kode_barang, 
										nama_barang, 
										deskripsi, 
										tgl_input, 
										harga_beli, 
	...
	elseif ($mod == "barang" AND $act == "edit") 
	{
		...
		mysqli_query($id_mysql,"UPDATE tb_barang SET nama_barang= '$nama_barang', 
										deskripsi= '$deskripsi', 
										tgl_input= '$tgl_input', 
	...
	{
		mysqli_query($id_mysql,"DELETE FROM tb_barang WHERE kode_barang = '$_GET[id]'") or die(mysqli_error());
