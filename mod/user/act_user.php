<?php
	...
	if ($mod == "user" AND $act == "edit") {
		...
		{
			mysqli_query($id_mysql,"UPDATE user SET nama_lengkap = '$_POST[nama_lengkap]',
										usernm = '$_POST[usernm]',
										level = '$_POST[level]',
										akses_master = '$master'
	...
	elseif($mod == "user" AND $act == "simpan")
	{
		if(!empty($_POST['passwd']))
		{
			if ($_POST['passwd'] == $_POST['rpasswd']) {
				$pass = md5($_POST['passwd']);
				mysqli_query($id_mysql,"INSERT INTO user(nama_lengkap,
										usernm,
										passwd,
										level)
									VALUES('$_POST[nama_lengkap]',
											'$_POST[usernm]',
											'$pass',
											'$_POST[level]')") or die(mysqli_error());
				flash('example_message', '<p>Berhasil menambah data user.</p>' );
