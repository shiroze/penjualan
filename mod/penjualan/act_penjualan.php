...
<?php
	...
	if($mod == "penjualan" AND $act == "add")
	{
		$cek_barang = mysqli_query($id_mysql,"SELECT * FROM tb_barang 
								WHERE kode_barang = '$_GET[id]'") or die(mysqli_error());
		...
		if (mysqli_num_rows($cek_barang) > 0) {
			...
			$cek_det_barang = mysqli_query($id_mysql,"SELECT * FROM tb_detail_penjualan_tmp 
										WHERE kode_barang = '$_GET[id]' 
										AND petugas = '$_SESSION[login_id]'") or die(mysqli_error());
			if(mysqli_num_rows($cek_det_barang) > 0)
			{
				mysqli_query($id_mysql,"UPDATE tb_detail_penjualan_tmp SET qty = qty + 1 
							WHERE kode_barang = '$_GET[id]' 
							AND petugas = '$_SESSION[login_id]'") or die(mysqli_error());
			}
			else
			{
				mysqli_query($id_mysql,"INSERT INTO tb_detail_penjualan_tmp (kode_barang,
																	harga,
																	disc,
																	qty,
																	petugas,
	...
	elseif ($mod == "penjualan" AND $act == "batal") {
		mysqli_query($id_mysql,"DELETE FROM tb_detail_penjualan_tmp 
					WHERE kode_barang = '$_GET[id]' 
					AND petugas = '$_SESSION[login_id]'") or die(mysqli_error());
	...
	{
		$qtmp = mysqli_query($id_mysql,"SELECT * FROM tb_detail_penjualan_tmp 
		...
		if (mysqli_num_rows($qtmp) > 0) {
			...
			//print_r($chart);
			$qpel = mysqli_query($id_mysql,"SELECT * FROM tb_pelanggan 
			...
			if (($total_bayar <= $jmlbayar) OR ($_POST['status'] == "HUTANG")) {
				...
				//pembuatan header
				$qsimpanheader = mysqli_query($id_mysql,"INSERT INTO tb_penjualan(no_transaksi,
																		kode_pelanggan, 
																		nama_pelanggan, 
																		tgl_transaksi, 
																		petugas, 
				...
				else
				{
					foreach ($chart as $row) {
						$qsimpandetail = mysqli_query($id_mysql,"INSERT INTO tb_detail_penjualan(no_transaksi,
																						kode_barang,
																						qty,
																						harga, 
																						disc, 
	...
	elseif ($mod == "penjualan" AND $act == "hapus") {
		...
		{
			mysqli_query($id_mysql,"DELETE FROM tb_penjualan WHERE no_transaksi = '$_GET[id]'") or die(mysqli_error());
