	...
	switch ($act) {
			...
			echo"<div class='w3-row-padding'>
				...
				<div class='w3-col s9'>
					...
					$sql = mysqli_query($id_mysql,"SELECT a.*, b.nama_barang, b.satuan 
										...
										ORDER BY timestmp ASC") or die(mysqli_error());
					$total = 0;
		...
		case "detail" :
			...
			if(isset($_GET['id']))
			{
				...
				$sqltrans = mysqli_query($id_mysql,"SELECT * FROM tb_pembelian WHERE no_faktur = '$_GET[id]'") or die(mysqli_error());
...
<script type="text/javascript">
	$(function(){
		...
		<?php
			$sqlTags = mysqli_query($id_mysql,"SELECT * FROM tb_barang 
								ORDER BY nama_barang ASC") or die(mysqli_error());
	    ...
	    <?php
			$sqlTags = mysqli_query($id_mysql,"SELECT * FROM tb_supplier 
								ORDER BY nama_toko ASC") or die(mysqli_error());
