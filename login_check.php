...
if (isset($_POST['submit'])) {
	...
	else
	{
		...
		$username = mysqli_real_escape_string($id_mysql,$username);
		$password = mysqli_real_escape_string($id_mysql,$password);
		// SQL query untuk memeriksa apakah karyawan terdapat di database?
		$query = mysqli_query($id_mysql,"SELECT * FROM user WHERE passwd='$pass' AND usernm='$username'") or die(mysqli_error());
		...
		if ($rows == 1) {
			...
			mysqli_query($id_mysql,"UPDATE user SET last_login = NOW() WHERE id_user = '$a[id_user]'");
