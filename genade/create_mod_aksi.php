...
$string = "<?php
	...
	if(\$mod == \"".$mod."\" AND \$act == \"simpan\")
	{
		...
		$string .="\n\n\t\tmysqli_query($id_mysql,\"INSERT INTO ".$nama_table."('".$pk."', ";
									$string .= implode(", ", $arr);
									$string .=")
									VALUES (\$".$pk.", ";
									$string .= implode(", ", $arr2);
									$string .=")\") or die(mysqli_error());
	...
	elseif (\$mod == \"".$mod."\" AND \$act == \"edit\") 
	{
		...
		$string .= "\n\n\t\tmysqli_query($id_mysql,\"UPDATE ".$nama_table." SET ";
						$string .= implode(", ", $arr3);
					$string .= " 
					WHERE ".$pk." = '\$_POST[id]'\") or die(mysqli_error());
	...
	{
		mysqli_query($id_mysql,\"DELETE FROM ".$nama_table." WHERE ".$pk." = '\$_GET[id]'\") or die(mysqli_error());
